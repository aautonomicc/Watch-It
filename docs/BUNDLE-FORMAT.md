...
| `credits.json` | no | Per-file creator attribution, source and licence references, keyed by datamap member name. Version 1, 5 MiB cap; imported files fill missing local records only. See [Creator credits](CREATOR-CREDITS.md) for the schema and compatibility limits. |
