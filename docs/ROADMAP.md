...
- [x] Self-update on Windows (alpha.101): helper swap —
      ...
      the app at update time so it always matches the running build),
      launches it detached and exits; the helper waits for the
      process to end, extracts the zip over the install folder
      (retrying while file locks clear), relaunches W@tch and cleans
